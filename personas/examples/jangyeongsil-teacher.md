...
**리뷰 기준선** — 교사 UI·API `persona` 예시로 쓸 때 이 수준의 구체성을 권장한다.
...
- `PERSONA_BIAS`: 연 발명 등 persona beliefs를 사실처럼 서술
- `RETRIEVAL_ERROR`: document_text와 어긋나는 검색·인용 뉘앙스 (예: 자격루 서양 기술 주장)
...
> 교사 UI placeholder·도움말 작성 시 `_template.md` 필드를 참고한다. 과제별 본문은 서비스 DB에만 저장한다.
